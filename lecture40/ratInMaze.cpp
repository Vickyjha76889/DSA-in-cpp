...
bool isvalid(int x , int y, const vector<vector<int>>& m ,  const vector<vector<int>> visited , int n){
...
void solve(vector<vector<int>>& m , int n , int x , int y , string& path, vector<vector<int>>& visited  , vector<string>& ans){
    ...
    int newx = x+1;
    int newy = y;
    if(isvalid(newx, newy, m, visited, n)){
        path.push_back('D');
...
vector<string> findPath(vector<vector<int>>& m , int n){
    ...
    int srcx = 0;
    int srcy = 0;
    if (m[0][0] == 1) // Only start if the first cell is open
